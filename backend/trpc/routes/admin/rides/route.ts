...
export default adminProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    let query = db
      .from("rides")
      .select(
        "id, userId, driverId, pickupAddress, dropoffAddress, rideType, status, fare, baseFare, minFare, maxFare, bookingFee, serviceFee, zoneFee, fareAdjustmentPercent, distance, duration, paymentMethod, createdAt, completedAt, cancelledAt",
        { count: "exact" }
      )
